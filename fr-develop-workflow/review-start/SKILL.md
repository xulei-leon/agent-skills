...
Run two independent reviews for the target file and save both review reports under `docs/4-Reviews/`: one with `opencode` and one with Claude Code.
...
|---|---|---|
| Document review, docs review, spec review, markdown review, plan review | `review` | `deepseek/deepseek-v4-pro` via `opencode` (report name: `deepseek-v4-pro`), `ark-code-latest` via Claude Code |
| Code review, implementation review, source review, diff review, PR-style review | `code-review` | `volcengine-agent-plan/glm-latest` via `opencode`, `ark-code-latest` via Claude Code |
...
docs/4-Reviews/api-code-review-by-glm-latest.md
...
4. Run `opencode` for `deepseek/deepseek-v4-pro` when doing document review, or `glm-latest` when doing code review.
5. Run Claude Code for `ark-code-latest`.
6. Ensure each model writes only its own report file.
7. After both commands finish, verify both report files exist and are non-empty.
...
```bash
opencode -m "glm-latest" --agent build run "Review the file: src/app.ts

Review type: code review

Write the complete review report to:
docs/4-Reviews/app-code-review-by-glm-latest.md
...
Report the two generated file paths to the user. If either command fails or a report file is missing or empty, state which model failed and include the relevant command output.
